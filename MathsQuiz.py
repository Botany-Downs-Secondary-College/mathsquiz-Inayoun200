...
class MathsQuiz:
  ...
  def __init__(self,parent):
    ...
    self.report_treeview = ttk.Treeview(parent)
...
#Error checking for empty and age limit cases
  ...
  def next_question(self):
    ...
    if self.index >= 6:
      ...
      self.report_treeview.grid(row=2, column=0)


      with open('scores.txt', 'a+') as scores_file:
        scores_file.write(self.name.get() + '' + self.age.get() + '' + str(self.score) + '' + datetime.date.today())
  ...
  def update_report_treeview(self):
    ...
    with open('scores.txt', 'r') as scores_file:
      ...
      scores.sort(key=lambda score: (score[3], score[2]), reverse=True)
      for score in scores:
